// 메모리 공간
/*
    Code 영역: 사람이 작성한 프로그램의 코드가 저장되는 공간

    Data 영역: 프로그램의 전역 변수와 정적 변수가 저장되는 영역으로, 시작과 함께 메모리에 할당이 되며
    프로그램이 종료 되어야만 메모리에서 사라지는 특성을 갖고 있다. [const char* 전역 변수/ 정적변수]

    Heap 영역: 사용자에 의해 메모리가 도적으로 할당되고 사용자가 직접 메모리를 해제해야 함

    Stack 영역: 함수의 호출과 함께 메모리에 할당되며 함수의 호출이 끝나면 메모리에서 사라지는 특성이 있음 [지역 변수와 매개 변수가 이에 포함됨]
*/
